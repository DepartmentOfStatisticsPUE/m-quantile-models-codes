glm.mq.poisson <-
           ...
           k = 1.6)
  {
               ...
               offset = offs)
      {
        ...
        g.objective <- function(beta)
        {
          ...
          apply(Xwith * as.vector(1 / nb /
                                    sqrt(V) * w.x * basepsi(r.stand) * deriv.mu),
                2,
                sum) - a.const
        }
    ...
    ifelse (weights.x, w.x <- sqrt(1 - hat(x)), w.x <-
              rep(1, length = n))
    assign("w.x", w.x)
    ...
    for (i in 1:length(q)) {
      ...
      for (iiter in 1:maxit) {
        ...
        a.j <- 2 * a.j * (q[i] * (r.stand > 0) + (1 - q[i]) * (r.stand <=
                                                                 0))
